...
class Dth(QDialog):
    ...
    def __init__(self):
        ...
        self.ui.btn_ticket_dth_support.clicked.connect(
            self.generateTicketSupport)
        self.ui.btn_clear_dth__support.clicked.connect(self.clearDTHSupport)
        self.ui.btn_clear_dth.clicked.connect(self.clearDHT)

        get_dolar = getDolar()
        self.ui.lblPriceReferentialDolarDTH.setText(
            "Precio del dolar del día %s, Bs. %.2f" % (_date, get_dolar))
    ...
    def getPlansByName(self, name):
        ...
        dict = {"id": id, "price_ves": price_ves,
                "price_usd": price_usd, "alias": alias}
        return dict
    ...
    def generateTicketsDTH(self):
        global aditional_, premium_tv_, total_ab
        try:
            ...
            premium = self.db.getProductsByIdAndTypeProduct(
                self.current_product_id, self.current_premium)
            aditionals = self.db.getProductsByIdAndTypeProduct(
                self.current_product_id, self.current_aditionals)
            only_ved_dth = True if self.ui.only_ved_dth.isChecked() else False
            only_usd_dth = True if self.ui.only_usd_dth.isChecked() else False
            dual_dth = True if self.ui.dual_dth.isChecked() else False
            ...
            if only_ved_dth:
                plan_label = plan_label.replace("$", "Bs")

            if only_usd_dth:
                plan_label = plan_label.replace("Bs", "$")

            price_ves = self.db.getPriceVES(
                plans_['price_ves'], plans_['price_usd'])
            price_usd = self.db.getPriceUSD(
                plans_['price_ves'], plans_['price_usd'])
            if plan_label == None:
                self.templateUndefined(getMessages(
                    "TEMPLATE_UNDEFINED") % combo_plans)
            else:
                if only_ved_dth:
                    plans_package = plan_label % price_ves
                if only_usd_dth:
                    plans_package = plan_label % price_usd

                if dual_dth:
                    plan_label = plan_label.replace("Bs. %.2f", "Bs. %.2f / $. %.2f")
                    plans_package = plan_label % (price_ves, price_usd)
            ...
            for i in range(0, premium.rowCount()):
                ...
                if checkbox.isChecked():
                    ...
                    if only_ved_dth:
                        template = template.replace("$", "Bs")

                    if only_usd_dth:
                        template = template.replace("Bs", "$")

                    price_ves = self.db.getPriceVES(
                        prices['price_ves'], prices['price_usd'])
                    price_usd = self.db.getPriceUSD(
                        prices['price_ves'], prices['price_usd'])
                    if template == None:
                        self.templateUndefined(getMessages(
                            "TEMPLATE_UNDEFINED") % name_package)
                        checkbox.setChecked(False)
                    else:
                        if only_ved_dth:
                            premium_tv_ = template % price_ves
                        if only_usd_dth:
                            premium_tv_ = template % price_usd

                        if dual_dth:
                            template = template.replace("Bs. %.2f", "Bs. %.2f / $. %.2f")
                            premium_tv_ = template % (price_ves, price_usd)
            ...
            for i in range(0, aditionals.rowCount()):
                ...
                if checkbox.isChecked():
                    ...
                    if template == None:
                        ...
                        if only_usd_dth:
                            aditional_ = template % (qty_, price_usd)

                        if dual_dth:
                            template = template.replace("Bs. %.2f", "Bs. %.2f /  $. %.2f")
                            aditional_ = template % (price_ves, price_usd)
            ...
            if only_ved_dth:
                total_ab = "Total Bs. %.2f. Todos nuestros precios incluyen IVA" % float(total)
            if only_usd_dth:
                total_ab = "Total $. %.2f. Todos nuestros precios incluyen IVA" % float(total_usd)

            if dual_dth:
                total_ab = "Total Bs. %.2f /  $. %.2f. Todos nuestros precios incluyen IVA" % (total, total_usd)


            text_ab = "%s%s%s%s%s" % (
                labels["PREFIX"], plans_package, premium_tv, aditional, total_ab)
            temp_ = "%s%s%s%s%s%s" % (
                labels_k2b["PREFIX"], plans_package, premium_tv, aditional, total_ves_k2b, labels_k2b["END"])
            ...
            self.ui.lbl_total_dth.setText(
                "Total Bs %.2f / $ %.2f" % (total, total_usd))
    ...
    def generatePremiumInter(self, product_id, type_product):
        ...
        for i in range(0, query.rowCount()):
            ...
            self.createCheckboxesPremiumInter(
                name, alias, position_x, position_y, with_box, heigth_box)
    ...
    def generatePremiumMovistar(self, product_id, type_product):
        ...
        for i in range(0, query.rowCount()):
            ...
            name = query.record(i).value("name")
            alias = query.record(i).value("alias")
            self.createCheckboxesPremiumMovistar(
                name, alias, position_x, position_y, with_box, heigth_box)
    ...
    def createCheckboxesPremiumMovistar(self, name, alias, position_x, position_y, with_box, heigth_box):
        self.ui.groupBoxPremiumDTHMovistar.setGeometry(
            QtCore.QRect(420, 60, 280, heigth_box))
    ...
    def createCheckboxesAditionnal(self, name, alias, position_x, position_y, with_box, heigth_box):
        self.ui.groupBoxAditionalsDTH.setGeometry(
            QtCore.QRect(20, 190, 360, heigth_box))
        self.checkBox = QtWidgets.QCheckBox(self.ui.groupBoxAditionalsDTH)
        self.checkBox.setGeometry(QtCore.QRect(
            position_x, position_y, 220, 30))
        self.checkBox.setObjectName(alias)
        self.checkBox.setText(name)
        ...
        self.spinBox.setGeometry(QtCore.QRect(
            position_x + 200, position_y, 50, 30))
        self.spinBox.setObjectName(alias + "_qty")

    def clearDHT(self):
        query = self.db.getProductsByIdAndTypeProduct(
            self.current_product_id, self.current_premium)
        for i in range(0, query.rowCount()):
            alias = query.record(i).value("alias")
            checkbox = self.findChild(QCheckBox, alias)
    ...
    def generateTicketSupport(self):
        ...
        if not self.ui.dth_coomand_not_send.isChecked():
            if self.ui.dth_refresh.isChecked():
                command = "%s %s %s + %s" % (
                    txt_k2b, labels["SEND_COMAND"], comand_action.upper(), labels["REFRESH"])
            else:
                if self.ui.combo_dth_failure.currentIndex() != 0:
                    command = txt_k2b + \
                        labels["SEND_COMAND"] + comand_action.upper() + ", "
        else:
        ...
        dth_potency = "" if self.ui.dth_potency.text(
        ) == "" else "P:%sdB/ " % self.ui.dth_potency.text()
        dth_quality = "" if self.ui.dth_quality.text(
        ) == "" else "C%s/ " % self.ui.dth_quality.text()
        dth_intensity = "" if self.ui.dth_intensity.text(
        ) == "" else "I:%s/" % self.ui.dth_intensity.text()
        dth_cnr = "" if self.ui.dth_cnr.text() == "" else "CNR:%s/ " % self.ui.dth_cnr.text()
        dth_vber = "" if self.ui.dth_vber.text(
        ) == "" else "VBER:%s/ " % self.ui.dth_vber.text()

        values = "%s%s%s%s%s%s," % (
            dth_potency, dth_quality, dth_intensity, dth_intensity, dth_cnr, dth_vber)

        txt_k2b = "%s%s%s%s%s%s%s" % (
            stb, failure, command, values, clear_weather, connections_ok, support)
    ...
    def changeFailure(self, state):
        ...
        if state == 0:
            self.ui.text_motive_dth.clear()
        if state == 1:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][0]["364_373"])
            self.ui.text_commands_dth.insertHtml(template['comands'][0]["no"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][1]["service"])
        if state == 2:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][1]["364_1127"])
            self.ui.text_commands_dth.insertHtml(
                template['comands'][1]["rescan_channels"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][0]["send_comand"])
        if state == 3:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][2]["364_250"])
            self.ui.text_commands_dth.insertHtml(
                template['comands'][2]["resend_key"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][0]["send_comand"])
        ...
        if state == 9:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][3]["364_1744"])
            self.ui.text_commands_dth.insertHtml(
                template['comands'][3]["update_pin"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][0]["send_comand"])
        if state == 10:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][2]["364_250"])
            self.ui.text_commands_dth.insertHtml(
                template['comands'][4]["refresh_recharge_inactive"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][0]["send_comand"])
        if state == 11:
            self.ui.text_motive_dth.insertHtml(
                template['motives'][2]["364_250"])
            self.ui.text_commands_dth.insertHtml(
                template['comands'][5]["refresh_recharge_active"])
            self.ui.txt_ticket_ab_dth_support.insertHtml(
                template['subscriber'][0]["send_comand"])
        if state == 12:
            self.ui.text_motive_dth.insertHtml(
    ...
    def removeDecoder(self):
        self.ui.dth_list_decoder.takeItem(
            self.ui.dth_list_decoder.currentRow())
